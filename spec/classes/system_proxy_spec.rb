...
describe 'system_proxy', :type=> :class do
  ...
  describe 'on a non-RHEL system' do
    context 'that needs a proxy installed' do
      ...
      it { should contain_system_proxy__env_var('HTTP_PROXY')
        .with_value('http://proxy.example.com:80')}
      it { should contain_system_proxy__env_var('http_proxy')
        .with_value('http://proxy.example.com:80')}
      it { should contain_system_proxy__env_var('PIP_PROXY')
        .with_value('http://proxy.example.com:80')}
    end
  ...
  describe 'on a RHEL system' do
    context 'that needs a proxy installed' do
      ...
      it { should contain_system_proxy__env_var('PIP_PROXY')
        .with_value('http://proxy.example.com:80')}
      it { should contain_class('system_proxy::redhat')
        .with_proxy_uri('http://proxy.example.com:80')}
    end
